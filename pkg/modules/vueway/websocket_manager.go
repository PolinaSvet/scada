...
// HandleMessConnection обрабатывает подключение WebSocket для сообщений
func (wm *WebSocketManager) HandleMessConnection(clientID string, userID string, clientType ClientType, w http.ResponseWriter, r *http.Request) error {
	conn, err := NewRealWebSocketConnection(w, r, clientID+"_mess")
	if err != nil {
		return err
	}

	wm.messWSConnections.Store(clientID, conn)
	log.Printf("Mess WebSocket connected: %s (user: %s, type: %v)", clientID, userID, clientType)
	return nil
}
...
// Mess
// SendToClient отправляет сообщение клиенту через WebSocket данных
func (wm *WebSocketManager) SendMessToClient(clientID string, message types.Message) error {
	connInterface, exists := wm.messWSConnections.Load(clientID)
	if !exists {
		return fmt.Errorf("client mess connection not found: %s", clientID)
	}

	conn, ok := connInterface.(WebSocketConnection)
	if !ok {
		return fmt.Errorf("invalid mess connection type for client: %s", clientID)
	}
...
// SendToAllClients отправляет сообщение всем подключенным клиентам
func (wm *WebSocketManager) SendMessToAllClients(message types.Message) {
	clientCount := 0
	wm.messWSConnections.Range(func(key, value interface{}) bool {
		clientID := key.(string)
		if wm.clientManager.CheckClientValidity(clientID) {
			if err := wm.SendMessToClient(clientID, message); err != nil {
				log.Printf("Error sending to client %s: %v", clientID, err)
				wm.CloseConnection(clientID)
			} else {
				clientCount++
...
// SendToClientByID отправляет сообщение конкретному клиенту
func (wm *WebSocketManager) SendMessToClientByID(clientID string, message types.Message) error {
	if clientID == "" {
		wm.SendMessToAllClients(message)
		return nil
	}

	return wm.SendMessToClient(clientID, message)
}
...
func (wm *WebSocketManager) StartWebSocketServers(ctx context.Context) {
	...
	go wm.startServer(ctx, wm.config.TestPort, wm.handleMessWebSocket)
}
...
func (wm *WebSocketManager) handleMessWebSocket(w http.ResponseWriter, r *http.Request) {
	clientID := r.URL.Query().Get("clientId")
	userID := r.URL.Query().Get("userId")
	clientType := r.URL.Query().Get("type")
	...
	if err := wm.HandleMessConnection(clientID, userID, cType, w, r); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
}
...
func (wm *WebSocketManager) CloseConnection(clientID string) {
	...
	connections := []*sync.Map{&wm.dataWSConnections, &wm.controlWSConnections, &wm.messWSConnections}
